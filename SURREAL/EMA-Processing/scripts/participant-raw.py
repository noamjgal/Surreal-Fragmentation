# participant-raw.py
# Process raw (unstandardized) EMA responses
import pandas as pd
import numpy as np
...
def process_responses(participant_data):
    """
    Process CES and STAI responses at the participant level without standardization.
    Just validates data and applies reverse scoring where needed.
    """
...
def main():
    ...
    output_dir = Path(project_root) / "output" / "raw"
    ...
    for file in data_dir.glob("participant_*.csv"):
        try:
            ...
            # Process responses (no standardization)
            processed_data = process_responses(participant_data)
            
            # Save processed data
            output_file = output_dir / f"raw_{file.name}"
            processed_data.to_csv(output_file, index=False)
            logging.info(f"Saved raw data to: {output_file}")
            
            # Create summaries for this participant
            scale_summary = create_ema_summary_by_scale(processed_data)
            variable_summary = create_ema_summary_by_variable(processed_data)
    ...
    if scale_summaries and variable_summaries:
        ...
        overall_scale_summary.to_csv(output_dir / "overall_raw_summary_by_scale.csv", index=False)
        
        # Combine and save variable-level summary
        overall_variable_summary = pd.concat(variable_summaries, ignore_index=True)
        overall_variable_summary.to_csv(output_dir / "overall_raw_summary_by_variable.csv", index=False)
        
        logging.info("Created overall raw summaries")
