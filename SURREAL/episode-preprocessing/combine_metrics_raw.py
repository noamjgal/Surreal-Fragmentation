...
def setup_logging():
    ...
    try:
        ...
        log_file = log_dir / f"combine_metrics_raw_report_{timestamp}.log"
...
def load_raw_ema_data(raw_dir):
    """Load all raw EMA data and create daily averages."""
    logging.info(f"Loading raw EMA data from {raw_dir}...")
    
    # Find all raw participant files
    raw_files = list(raw_dir.glob("raw_participant_*.csv"))
    logging.info(f"Found {len(raw_files)} raw participant files")
    ...
    for file in raw_files:
        try:
            ...
            participant_id = file.stem.replace('raw_participant_', '')
            ...
            for scale in ['STAI-Y-A-6', 'CES-D-8']:
                ...
                if not scale_data.empty:
                    # Group by date and calculate mean raw scores
                    ...
                    daily_scale = daily_scale.rename(columns={
                        'score_reversed': f'{scale}_reversed',
                        'Response Key': f'{scale}_raw'
                    })
...
def merge_ema_and_fragmentation(ema_data, frag_data):
    ...
    # Validate the merged data - adjust ranges for raw scores
    validation_rules = {
        'numeric_ranges': {
            'STAI-Y-A-6_reversed': (1, 4) if 'STAI-Y-A-6_reversed' in merged_data.columns else None,
            'CES-D-8_reversed': (1, 5) if 'CES-D-8_reversed' in merged_data.columns else None,
            'STAI-Y-A-6_raw': (1, 4) if 'STAI-Y-A-6_raw' in merged_data.columns else None,
            'CES-D-8_raw': (1, 5) if 'CES-D-8_raw' in merged_data.columns else None,
...
def main():
    ...
    try:
        # Define path for raw EMA data
        raw_ema_dir = Path("/Users/noamgal/DSProjects/Fragmentation/SURREAL/EMA-Processing/output/raw")
        ...
        # Output directory
        output_dir = PROCESSED_DATA_DIR / "daily_ema_fragmentation_unstd"
        ...
        # Process raw EMA data
        logging.info(f"\n{'='*40}\nProcessing raw (unstandardized) EMA data\n{'='*40}")
        
        # Load EMA data
        logging.info(f"Loading raw EMA data from {raw_ema_dir}")
        daily_ema = load_raw_ema_data(raw_ema_dir)
        
        if daily_ema is None or daily_ema.empty:
            logging.error(f"Failed to load valid raw EMA data. Exiting.")
            return
            
        logging.info(f"Loaded {len(daily_ema)} daily raw EMA records")
        ...
        logging.info(f"Found {len(ema_participants)} unique participants in raw EMA data")
        ...
        logging.info(f"Processing fragmentation records for raw EMA data...")
        ...
        if matched_records:
            ...
            logging.info(f"Created {len(combined_data)} matched records for raw EMA data")
            
            # Log match types
            match_type_counts = combined_data['match_type'].value_counts()
            logging.info(f"Match types: {match_type_counts.to_dict()}")
            
            # Save to CSV with raw indicator in filename
            output_file = output_dir / "combined_metrics_raw.csv"
            combined_data.to_csv(output_file, index=False)
            logging.info(f"Saved raw combined data to {output_file}")
            
            # Log the absolute path of the output file for easier access
            output_absolute_path = output_file.absolute()
            logging.info(f"\n{'*'*80}\nMAIN OUTPUT FILE SAVED TO:\n{output_absolute_path}\n{'*'*80}")
            # Also print directly to terminal to ensure visibility
            print(f"\n{'*'*80}\nMAIN OUTPUT FILE SAVED TO:\n{output_absolute_path}\n{'*'*80}")
        else:
            logging.warning(f"No matches found for raw EMA data! Could not create combined dataset.")
        
        # Log discard statistics
        logging.info(f"\n{'='*50}")
        logging.info(f"MATCH STATISTICS FOR RAW EMA DATA:")
        ...
        if discard_stats['no_participant'] > 0 or discard_stats['no_date'] > 0:
            logging.info(f"\nDETAILED MISMATCH REPORT FOR RAW EMA DATA:")
        ...
        # Add a CLEAR summary about output file at the end
        if 'combined_data' in locals() and len(combined_data) > 0:
            output_file = output_dir / "combined_metrics_raw.csv"
            output_absolute_path = output_file.absolute()
            summary_message = f"\n{'#'*80}\nOUTPUT SUMMARY:\n- Records processed: {len(combined_data)}\n- OUTPUT FILE: {output_absolute_path}\n{'#'*80}\n"
        else:
            summary_message = f"\n{'#'*80}\nOUTPUT SUMMARY:\n- NO OUTPUT FILE CREATED - No matching records found\n{'#'*80}\n"
        
        # Log and print the summary message
